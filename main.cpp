...
int main(int argc, char *argv[])
{
  ...
  double w, h; //temporarily used to store the returned width and height
  EMSCRIPTEN_RESULT r = emscripten_get_element_css_size("canvas", &w, &h); //get the width and height of the viewport

  viewport_width_ = (int)w; //cast these (viewport height and width) to ints and store them
  viewport_height_ = (int)h;

  pixel_ratio_ = emscripten_get_device_pixel_ratio(); //get a scaling factor for the screen

  pixel_width_  = round(w * pixel_ratio_); //these should correlate with the actual number of pixels across and up the screen
  pixel_height_ = round(h * pixel_ratio_);
...
//  memset(data, 0, pixel_width_ * pixel_height_ * 4);
  ...
  emscripten_set_main_loop(generate_frame, 0, 0); //begin main program loop
}
